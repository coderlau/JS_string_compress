...
stringCompressor.comp = function(txt) {
	...
	if(this.debug){console.log("rawLib = ", rawLib)}
	...
	if(this.debug){console.log("newLib = ", newLib, " newLibFreq = ", newLibFreq)}
	...
	if(this.debug){console.log("newLib after filter = ", newLib)}
	...
	function sortWordListsLongestFirst(arr) {
		...
		//if(this.debug){console.log("totalCompared = ", totalCompared, " totalSwapped = ", totalSwapped)}
	}

	sortWordListsLongestFirst(newLib)
	if(this.debug){console.log("newLib after sort = ", newLib)}
	...
	if(this.debug){console.log("newLibLocations absolute = ", newLibLocations, " left over egString = ", egString)}
	...
	if(this.debug){console.log("newLibLocations2 relative = ", newLibLocations2)}
	...
	if(this.debug){console.log('newLibLocations2 relative base36 = ', newLibLocations2)}
	...
	if(this.debug){console.log('locs = ', locs, " locs.length = ", locs.length)}
	...
	if(this.debug){console.log('compString = ', compString)}
	if(this.debug){console.log('ratio = ', compString.length / txt.length)}
...
stringCompressor.uncomp = function(txt) {
	...
	//extract lib
	var split = txt.split(' ')
	if(this.debug){console.log("uncomp; string.split = ", split)}
	var lib = new Array()
	for(var i = 0; i <= split[0] - 1 ; i++){
		lib.push(split[i+1])
	}
	if(this.debug){console.log("uncomp; lib = ", lib)}
	
	//extract locsTxt
	txt = split.slice(lib.length+2).join(' ')
	if(this.debug){console.log("uncomp; txt after extracted lib = ", txt)}
	var locsTxt = txt.slice(0, split[lib.length+1]) //split[lib.length+1] is the locs.length in .comp
	if(this.debug){console.log("uncomp; locsTxt = ", locsTxt)}
	...
	if(this.debug){console.log("locsArr base 36 and relative = ", locsArr)}
	...
	if(this.debug){console.log('locsArr base 10 and relative = ', locsArr)}
	...
	if(this.debug){console.log('locsArr base 10 and absolute = ', locsArr)}

	//extract leftovers
	var leftover = txt.replace(locsTxt, '')
	if(this.debug){console.log("uncomp; leftover = ", leftover, ' length = ', leftover.length)}
	...
	if(this.debug){console.log("rebuiltString = ", rebuiltString)}
